...
public class MainMenuScreen extends SerpScreen {
	...
	public MainMenuScreen(Game game) {
		...
		Gdx.app.log("MainMenuScreen",
				"Constructor: going forward constructing...");
		Gdx.app.log("MainMenuScreen", "MainMenuScreen(game), this.assets: "
				+ this.assets);
	...
	public MainMenuScreen(Game game, AssetManager assets) {
		...
		Gdx.app.log("MainMenuScreen",
				"Constructor: going forward constructing...");
		Gdx.app.log("MainMenuScreen",
				"MainMenuScreen(game, assets), this.assets: " + this.assets);
		Gdx.app.log("MainMenuScreen", "MainMenuScreen(game, assets), assets: "
				+ assets);
	...
	public void render(float delta) {
		...
		spriteBatch.draw(mainMenu, 64 * ppuX, 132 * ppuY, 192 * ppuX,
				128 * ppuY); // -220 - 128 = (menu height)
	...
	private void inputController() {
		...
		Gdx.app.log("MainMenuScreen",
				"inputController(), starting to process input");
		...
		if (Gdx.input.justTouched()) {
			...
			if (inBounds(touchPos, 64 * ppuX, 220 * ppuY, 192 * ppuX, 42 * ppuY)) {
				Gdx.app.log("MainMenuScreen",
						"inputController(), play touched!");
		
				Gdx.app.log("MainMenuScreen",
						"inputController(), going to GameScreen");
				if (this.assets == null)
			...
			if (inBounds(touchPos, 64 * ppuX, 262 * ppuY, 192 * ppuX, 42 * ppuY)) { 
				
				Gdx.app.log("MainMenuScreen",
						"inputController(), highscores touched!");
				Gdx.app.log("MainMenuScreen",
						"inputController(), going to HighscoreScreen");
				if (this.assets == null)
	...
	public void drawText(String line, int x, int y) {
		...
		Gdx.app.log("MainMenuScreen",
				"starting drawText(), will get numbers.png now");
	}
