...
public abstract class SerpScreen implements Screen {
	...
	protected boolean inBounds(Vector3 touchPos, float x, float y, float width,
			float height) {
		Gdx.app.log("SerpScreen", "starting calculate inBounds()");
		if (touchPos.x > x && touchPos.x < x + width - 1 && touchPos.y > y
				&& touchPos.y < y + height - 1) {
			Gdx.app.log("SerpScreen",
					"ended calculate inBounds() with true result");
			return true;
		} else {
			Gdx.app.log("SerpScreen",
					"ended calculate inBounds() with false result");
			return false;
		}
	}
