...
public class GameScreen extends SerpScreen implements ActionListener {
	...
	private void drawWorld(World world) {
		...
		spriteBatch.draw(headPixmap, x * ppuX - (headPixmap.getWidth() / 2)
				* ppuX, y * ppuY - (headPixmap.getHeight() / 2) * ppuY,
				42 * ppuX, 42 * ppuY);
	}
	...
	private void drawReadyUI() {
		...
		spriteBatch.draw(ready, width / 2 - 112 * ppuX, height / 2 - 48 * ppuY,
				225 * ppuX, 96 * ppuY);
	}
	...
	private void inputRunning(float deltaTime) {
		...
		if ((Gdx.input.justTouched())) {
			...
			//AICI o sa implementez functionalitatea de KEYBOARD PRESS KEY
			else if (
				inBounds(touchPos, 0, height - 64 * ppuY, 64 * ppuX,	64 * ppuY) || miscare == -1 ) 
			{
				snake.turnLeft();
				
			} else if (inBounds(touchPos, width - 64 * ppuX, height - 64 * ppuY, 64 * ppuX, 64 * ppuY) || miscare == 1) 
	...
	private void drawRunningUI() {
		...
		spriteBatch.draw(buttons, 0, 0, 64 * ppuX, 64 * ppuY, 64, 64, 64, 64,
				false, false);
	...
	private void inputPaused() {
		...
		if ((Gdx.input.justTouched())) {
			Vector3 touchPos = new Vector3();
			touchPos.set(Gdx.input.getX(), Gdx.input.getY(), 0);
			if (inBounds(touchPos, width / 2 - 80 * ppuX, height / 2 - 48
					* ppuY, 160 * ppuX, 48 * ppuY)) {
				
				state = GameState.Running;
				return;
			}

			else if (inBounds(touchPos, width / 2 - 80 * ppuX, height / 2,
					160 * ppuX, 48 * ppuY)) {
	...
	private void drawPausedUI() {
		...
		spriteBatch.draw(pause, width / 2 - 80 * ppuX, height / 2 - 48 * ppuY,
				160 * ppuX, 96 * ppuY);
	}
	...
	private void inputGameOver() {
		if ((Gdx.input.justTouched())) {
			Vector3 touchPos = new Vector3();
			touchPos.set(Gdx.input.getX(), Gdx.input.getY(), 0);
			if (inBounds(touchPos, width / 2 - 32 * ppuX, (height / 3) * 2 - 32
					* ppuY, 64 * ppuX, 64 * ppuY)) {
	...
	private void drawGameOverUI() {
		...
		spriteBatch.draw(gameover, width / 2 - 98 * ppuX, (height / 3) * 2 - 25
				* ppuY, 196 * ppuX, 50 * ppuY);
		spriteBatch.draw(buttons, width / 2 - 32 * ppuX,
				height / 3 - 32 * ppuY, 64 * ppuX, 64 * ppuY, 0, 128, 64, 64,
				false, false);
	}
